...
// validateDefrag checks the defragmentation options. The thresholds are checked
// whether or not the feature is on, since a nonsensical value is a typo either
// way; the mode and runtime requirements only matter when it is on.
func (c Config) validateDefrag() error {
	if c.DefragIntervalSeconds < 1 {
		return fmt.Errorf("invalid defragIntervalSeconds %d: must be at least 1", c.DefragIntervalSeconds)
	}
	if c.DefragMaxMovesPerPass < 1 {
		return fmt.Errorf("invalid defragMaxMovesPerPass %d: must be at least 1", c.DefragMaxMovesPerPass)
	}
	if c.DefragMinGain < 1 {
		return fmt.Errorf("invalid defragMinGain %d: must be at least 1", c.DefragMinGain)
	}
	if c.DefragClaimCooldownSeconds < 0 {
		return fmt.Errorf("invalid defragClaimCooldownSeconds %d: must not be negative", c.DefragClaimCooldownSeconds)
	}
	if !c.DefragEnabled {
		return nil
	}
