...
type Config struct {
	...
	// field stays fleet-wide policy. A node selects its profile with the
	// ProfileLabel label; without the label the fleet-wide values apply. Every
	// profile is validated on every node, so a typo in any of them fails the
	// whole fleet fast instead of one node quietly.
...
func (p Profile) String() string {
	...
	return fmt.Sprintf("{reservedCPUs: %s}", format(p.ReservedCPUs))
}
