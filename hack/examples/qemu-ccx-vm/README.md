...
and set `CONTROL_CPUS` instead. It sets that many of the claim's own CPUs aside
for them, and the launcher runs the VM on the rest: it counts CPUs, not cores,
so `2` is one core on an SMT2 part, and a value that is not a multiple of the
SMT width splits one, because the tail it carves is whole cores only at those
sizes.

One claim with two requests and a vcpus claim beside a helpers claim give the
container the same CPUs, the union of every claim it holds. Either way the
launcher tells the two sets apart from the helpers request's metadata file,
never from `DRA_CPUSET_*`. These manifests use one claim.
