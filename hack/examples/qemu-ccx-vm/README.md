...
cannot be pinned outside its own cgroup cpuset -- so `CONTROL_CPUS` sets that
many of the claim's own CPUs aside for them, and the launcher runs the VM on the
rest. It counts CPUs, not cores: `2` is one core on an SMT2 part, and a value
that is not a multiple of the SMT width splits one, because the tail it carves
is whole cores only at those sizes. Keeping the vCPU count instead means a
larger claim and a selector to match.
