...
- `internal/driverconfig`: `DefragEnabled` and `validateDefrag`; the `Profiles`
...
`placements.go`, `deviceorder.go`, `pkg/cpuinfo/coretopology.go`) are visible to `git diff --stat` on
their own and are not repeated here.
