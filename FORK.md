...
- `pkg/driver/dra_hooks.go`: `cdiEnvValue`

- `pkg/driver/nri_hooks.go`: `draEnvEntry`

- `pkg/store/cpu_allocation.go`: `GetResourceClaimAllocationUnion`, `BeginRebind`, `CommitRebind`,
  `AbortRebind`, `GetRebindOrigin`, `ResourceClaimAllocations`
...
- `test/e2e`: the defragmentation suite, the claim-pod helpers in `e2e_suite_test.go`, and the
  `defragEnabled` config read-back
