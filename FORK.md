...
- `pkg/driver/driver.go`: the `applyMu`, `defrag`, `sysfs`, `pendingRounds`, `activeExactPlans`,
  ...
  `claimReader`, `namespace`, `placementWriters`, `placementWritersMu` and `placementPolicy` fields,
  `deviceTopology.deviceIsPool`, `deviceTopology.deviceNameToPartition`,
  `deviceTopology.deviceNameToUncoreCacheID`, `devicePartition`, `Providers.CgroupFS` and
...
- `pkg/driver/nri_hooks.go`: `draEnvEntry`, `exclusiveClaimUIDs`, `sharedContainerCPUs`,
  ...
  `cpusetUpdate`, `observedContainer`, `reportForeignCPUs`, `allocatedClaimsByUID`,
...
`status.go`, `slicewatch.go`, `pkg/cpuinfo/coretopology.go`, `pkg/device/partition.go`,
`api/attributes.go`,
