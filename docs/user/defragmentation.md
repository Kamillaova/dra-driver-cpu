...
All four knobs are documented in [Configuration](configuration.md#driver-configuration). The defaults
are deliberately cautious, because no measurements exist to tune them against yet: they prefer leaving
a node slightly fragmented over moving workloads often.

- `defragIntervalSeconds` (300) — the backstop for a node nothing else disturbs. Preparing or
  releasing a claim also triggers a pass.
- `defragMaxMovesPerPass` (4) — how much of a node one pass may disturb.
- `defragMinGain` (1) — the smallest improvement, in uncore caches needlessly spanned, worth moving
  anything for.
- `defragClaimCooldownSeconds` (600) — how long a claim is left alone after being moved.
...
themselves pinned, under cooldown, or there is no slack to move them through. A `movableIn` on a claim
