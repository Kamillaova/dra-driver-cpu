...
- **CRI-O v1.36** still vendors NRI v0.12.0 and is **not** supported.

A runtime may separately report no `Container.CDIDevices`, the field this driver uses to verify that a
claim a container names really was injected into it. Where it is empty the driver authenticates the
container against the claims it prepared for that pod instead. That fallback keys on the empty field
rather than on a runtime's name, because CRI-O is not the only runtime that leaves it so.
...
Passes are node-local and write only what belongs to their own node, so 100 nodes are 100 independent
instances with no coordination between them and no cluster-scale load. The flip side is that a planner bug reaches every
