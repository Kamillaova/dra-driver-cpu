...
- Requires `cpuDeviceMode: grouped`. `reservedCPUs` or `sharedPoolCPUs` in the same scope is an error
  rather than a merge: each describes the same CPUs from the other end, and a `reserved` or `shared`
  partition is how the list says it.
