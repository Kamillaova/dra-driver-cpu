...
- **CDI (Container Device Interface)**: The driver uses CDI to communicate the allocated CPU set to the container runtime.
  ...
  - For a claim that states [`cpuConfig.relocatable: true`](opaque-cpuset-overrides.md) the value is the literal string `dynamic` rather than a cpuset, since a claim whose placement may change has no cpuset to name for the life of its container. A workload that parsed the value now fails loudly instead of quietly pinning itself to CPUs its claim has left. A claim that permits no move keeps its cpuset in the value, because that cpuset is true for as long as its containers run.
