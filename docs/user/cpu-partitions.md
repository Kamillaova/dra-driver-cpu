...
— or under `resourceclaimtemplates/<podClaimName>/` where a template generated the claim — beside the
...
Pool devices publish no node-allocatable mapping. Every claim that asks for a pool holds the same
CPUs, so a mapping would subtract those cores from the node once per claim.
...
core. Nothing in the driver can produce that: the platform takes the siblings offline, by writing
`0` to `/sys/devices/system/cpu/cpuN/online` for each of them, and the driver checks that it did. On
Talos, for instance, those files are `machine.sysfs` keys, dotted and relative to `/sys`:
...
When the check fails, the driver names the exact files that would satisfy it, in its log and in a
...
different. Set them on the kernel command line (on Talos, `machine.install.extraKernelArgs`), which
needs a reboot.
...
1. Apply the configuration change (the partition list, the CPUs taken offline, or both).
...
## A fleet of more than one node type

The cpuset above describes one part. `"8-15"` is a whole CCX there and something else on the next
machine, so a fleet mixing node types writes one
[profile](configuration.md#driverconfig-sub-fields) per type and labels each node with the profile
matching its hardware:
...
```console
kubectl label node --selector=node.kubernetes.io/instance-type=r7625 dra.cpu/profile=r7625
```

Once any profile exists, a partition list outside one is an error and a node without the label
refuses to start: the failure a fleet-wide cpuset produces otherwise is a node running on another
part's core numbering, which nothing downstream can detect. A node type that carves out nothing is
labelled `dra.cpu/profile=default` and keeps every core in the implicit partition.
