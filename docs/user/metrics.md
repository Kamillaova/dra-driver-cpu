...
| `dra_cpu_synchronize_foreign_cpus_total`        | Counter   | none                            | Claim-holding containers `Synchronize` found running on CPUs another claim holds, each converged onto the CPUs its own claim holds. Rises when a driver died before pinning a container, which the runtime then leaves on whatever it started with.                                                                       |
| `dra_cpu_started_containers_converged_total`    | Counter   | none                            | Containers found, when they started, on CPUs their claims no longer hold, each pinned onto the CPUs its claims do hold. Rises when a defragmentation pass moves a claim while one of its containers is between creation and start, which is a normal race on a node that defragments, not a fault.                        |
...
| `dra_cpu_capacity_mirror_retracted_by_absence`  | Gauge     | none                            | Prepared claims whose capacity correction is withdrawn because a projection newer than the one they were first seen in no longer lists them. A projector that omits a claim it should be listing shows up here, and what it costs is capacity the node holds back rather than hands out. See [The Capacity Mirror](capacity-mirror.md). |
