...
| `Repairable` | Workload prefers alignment. May start split if the repair frontier is reachable ($1..3$ rounds) and confirmed by a witness at Prepare; runtime defragmentation will consolidate it into a whole cache. | Requires `relocatable: true` and a request whose `firstAvailable` offers a split alternative. Without one there is nothing to repair from: the CCXAlign plugin's Filter rejects the claim on every node, and the driver refuses it at Prepare. |
...
The 4 and 8 vCPU shapes offer no split alternative, so they must leave `cpuConfig.alignment` unset: `Repairable`
on them is refused.
...
>
> A cache device is shareable, but that does not make `count: 12` reachable from 8 devices: the allocator gives a
> request that many *distinct* devices, and only separate requests or claims ever share one. Shareability is what
> lets several claims sit in one cache, not what lets one claim count a cache twice.

### 4. Exact Partition Tolerations
When targeting devices in a named CPU partition, claims must declare an exact toleration. A toleration belongs
to the request that selects the devices, so a `firstAvailable` claim declares it in every subrequest:
```yaml
spec:
  devices:
    requests:
    - name: cpu
      firstAvailable:
      - name: aligned-1
        deviceClassName: dra.cpu-compute
        allocationMode: ExactCount
        count: 1
        capacity:
          requests:
...
### 5. CPU Requests and Limits
With `DRANodeAllocatableResources` enabled and the driver publishing its node-allocatable mapping, the scheduler
already counts the claim's CPUs: set no `cpu` request or limit, only `memory`. A `cpu` request would count the
same CPUs twice, and a `cpu` limit sets a CFS quota that every thread in the container spends, vCPUs and helper
threads alike, so a busy VM is throttled.

While the gate is disabled the scheduler is blind to DRA allocations, and the claim's CPUs must be mirrored into
the pod's requests as [Workload Configuration Requirements](workload-requirements.md) describes.
