...
> and apply under every grouping but `machine`: a claim there names its cores in `cpuset`, and a
> claim naming its cores cannot be relocatable.
...
- `BestEffort` runs where the allocator placed it, split or not, with no promise that it will be
  made whole; if it is relocatable, defragmentation may still move it. This is the plain exclusive
