...
func (cp *CPUDriver) frontier() (map[string]string, map[string]string) {
	...
	for _, partition := range partitions {
		...
		for _, numaNodeID := range numaNodes {
			...
			// A frontier field says how few rounds a repair of that size would
			// take. With no defragmenter to run them the honest answer is that
			// none is reachable, whatever the topology would allow: the
			// attribute is a promise about this node, not a property of its
			// caches. Published rather than omitted, because a claim selecting
			// on an attribute a device lacks fails to evaluate rather than
			// failing to match.
			if !cp.defrag.enabled || cp.nodeIsPoisoned(numaNodeID) {
