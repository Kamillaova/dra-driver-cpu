...
func TestSendDefragBatchKeepsOneBatchOutAtATime(t *testing.T) {
	// Giving up on a batch does not end it: the NRI stub calls the runtime with a
	// background context and nothing here can cancel it. A second batch sent
	// while the first is still blocked would leave a goroutine per attempt for
	// the life of the process, and would write a second cpuset for containers the
	// first call may yet move.
	d := exchangeDriver(t)
	d.defrag.batchTimeout = 20 * time.Millisecond
	release := make(chan struct{})
	var once sync.Once
	releaseRuntime := func() { once.Do(func() { close(release) }) }
	t.Cleanup(releaseRuntime)
	d.updater.onUpdate = func([]*api.ContainerUpdate) { <-release }

	updates := []*api.ContainerUpdate{{ContainerId: "ctr-1"}}
	logger := testr.New(t)

	_, err := d.sendDefragBatch(logger, updates)
	require.Error(t, err, "the first batch runs out of its own deadline")
	require.NotErrorIs(t, err, errDefragBatchInFlight)

	_, err = d.sendDefragBatch(logger, updates)
	require.ErrorIs(t, err, errDefragBatchInFlight)
	require.Len(t, d.updater.allCalls(), 1, "the runtime must not be called again while its answer is outstanding")

	// Once the runtime answers, however late, the next batch goes out.
	releaseRuntime()
	require.Eventually(t, func() bool { return !d.defragBatchInFlight.Load() }, time.Second, time.Millisecond)
	_, err = d.sendDefragBatch(logger, updates)
	require.NoError(t, err)
	require.Len(t, d.updater.allCalls(), 2)
}

func TestSendDefragBatchIsClearOnceItAnswers(t *testing.T) {
	// settleExchanges resends a refused half, and rollBackExchange its rollback,
	// the moment the previous batch answers, so the flag must be clear by then.
	d := exchangeDriver(t)
	d.defrag.batchTimeout = time.Second
	updates := []*api.ContainerUpdate{{ContainerId: "ctr-1"}}
	logger := testr.New(t)
	...
	for range 20000 {
		_, err := d.sendDefragBatch(logger, updates)
		require.NoError(t, err)
		require.False(t, d.defragBatchInFlight.Load(), "a batch that has answered must no longer read as out")
	}
}
