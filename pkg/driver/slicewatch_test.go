...
//
// Taints come back stamped, because that is what the API server does with one
// that arrives without a time: "Added automatically during create or update if
// not set". A fake that stored them unstamped was more faithful to the driver
// than to the server, and it is what let B113 through -- on a real node every
// tainted device disagreed with itself forever.
func storedFrom(plugin *mockKubeletPlugin) []*resourceapi.ResourceSlice {
	if plugin.publishedResources == nil {
		return nil
	}
	nodeName := testNodeName
	stamped := metav1.NewTime(time.Date(2026, 9, 18, 8, 24, 24, 0, time.UTC))
	...
	for i, slice := range pool.Slices {
		devices := slices.Clone(slice.Devices)
		for d := range devices {
			if len(devices[d].Taints) == 0 {
				continue
			}
			taints := slices.Clone(devices[d].Taints)
			for tIdx := range taints {
				if taints[tIdx].TimeAdded == nil {
					taints[tIdx].TimeAdded = &stamped
				}
			}
			devices[d].Taints = taints
		}
...
// TestAwaitStoredShrinkAcceptsAStampedTaint: the API server adds a time to a
// taint that arrives without one, and the driver never publishes one, so a
// comparison that weighed it would leave every tainted device -- which is every
// device of every named partition -- disagreeing with itself for as long as the
// driver ran, and no round could ever move a claim there.
func TestAwaitStoredShrinkAcceptsAStampedTaint(t *testing.T) {
	cp, plugin := newShrinkDriver(t, true)
	cp.storedSlices = storedSliceFunc(func() ([]*resourceapi.ResourceSlice, error) { return storedFrom(plugin), nil })

	// A squatter puts cache 1 below the floor its request policy sets, so the
	// driver publishes it at the floor with a taint, and the stored copy comes
	// back with the server's timestamp on that taint.
	placeCharged(t, cp, "claim-squatter", cacheDevice(0), cpuset.New(3))
	round := beginTestMove(t, cp, "claim-a", cacheDevice(0), cpuset.New(0, 8), cpuset.New(2, 10))

	require.NoError(t, cp.awaitStoredShrink(context.Background(), round),
		"a stored taint that differs only by the time the server stamped on it is the taint the driver published")
	require.Equal(t, []string{devattr.FloorTaintKey}, publishedTaintKeys(t, plugin, cacheDevice(1)),
		"the device really was tainted, so the comparison was the thing under test")
}
