			...
			if len(claimUIDs) == 0 {
				...
				for _, ref := range refs {
					...
					if err == nil && rec.Round != nil && rec.Round.RoundID != "" {
						hadRound = true
						origin, target := roundCPUsOf(rec, ref.Request)
...
func revertRoundOrigin(record *store.ClaimRecord) bool {
	origin, ok := roundOriginByRequest(*record)
	if !ok {
		return false
	}
...
//
// The origin is spread over the exclusive requests in name order with each
// keeping the size it has, which is the reverse of how the target was spread
// over them; a request that is not exclusive is absent, since a pool never
// moves.
func roundOriginByRequest(record store.ClaimRecord) (map[string]cpuset.CPUSet, bool) {
	if record.Round == nil {
		return nil, false
	}
	...
	for _, request := range moved {
		...
		byRequest[request.Request] = cpuset.New(cpuIDs[:size]...)
		cpuIDs = cpuIDs[size:]
	}
...
func roundCPUsOf(record store.ClaimRecord, request string) (cpuset.CPUSet, cpuset.CPUSet) {
