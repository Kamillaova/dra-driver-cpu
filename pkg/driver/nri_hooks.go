...
func (cp *CPUDriver) Synchronize(ctx context.Context, pods []*api.PodSandbox, containers []*api.Container) (rupdates []*api.ContainerUpdate, rerr error) {
	...
	for _, pod := range pods {
		...
		for _, container := range containers {
			...
			if len(claimUIDs) == 0 {
				...
				var kernelErr error
				if cgroupsPath == "" {
					kernelErr = fmt.Errorf("the runtime reported no cgroups path for this container")
				} else if cp.cgroupfs == nil {
					kernelErr = fmt.Errorf("this driver reads no cgroup tree: it mounts one only for defragmentation")
				} else {
	...
	reportForeignCPUs(cp.metrics, cpuAllocationStore, observed)
...
func reportForeignCPUs(metrics Recorder, allocations *store.CPUAllocation, observed []observedContainer) {
	if len(observed) == 0 {
		return
	}
	claimed := cpuset.New()
	...
	for _, container := range observed {
		...
		metrics.RecordSynchronizeForeignCPUs()
