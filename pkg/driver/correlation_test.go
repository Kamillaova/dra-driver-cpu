...
func TestBuildClaimCorrelation(t *testing.T) {
	...
	// The witness sits in the claim status beside the frontier the claim was
	// admitted against, and the frontier counts rounds. An exchange is two moves
	// inside one round, so counting moves makes the two disagree exactly when a
	// plan swaps -- which is the case an operator compares them in.
	exchange := &defrag.ExactPlan{
		Moves: []defrag.Move{
			{ClaimUID: "claim-1", From: cpuset.New(0, 1), To: cpuset.New(2, 3), Exchange: 1},
			{ClaimUID: "claim-2", From: cpuset.New(2, 3), To: cpuset.New(0, 1), Exchange: 1},
		},
	}
	swapCorr := cp.buildClaimCorrelation(claim, cpuset.New(0, 1), exchange)
	require.NotNil(t, swapCorr.WitnessRounds)
	require.Equal(t, 1, *swapCorr.WitnessRounds)
}

func TestAdmissionEventMessageNamesTheNUMANodeAndCaches(t *testing.T) {
	node := 0
	corr := store.ClaimCorrelation{NUMANode: &node, Partition: "dpdk", InitialCPUSet: "6-7", FrontierSnapshot: "-,-,-,-"}
	require.Equal(t, "admitted on NUMA 0 caches 1 partition dpdk with cpuset 6-7 frontier -,-,-,- witness ", admissionEventMessage(corr, []int{1}))
	require.Contains(t, admissionEventMessage(corr, []int{0, 1}), "caches 0,1 partition")

	corr.NUMANode = nil
	require.Contains(t, admissionEventMessage(corr, nil), "admitted on NUMA unknown caches unknown partition")
}
...
func TestPublishClaimPlacementStatusWithCorrelation(t *testing.T) {
	...
	err := driver.doPublishClaimPlacementStatus(context.Background(), testr.New(t), "claim-test", types.NamespacedName{})
	require.NoError(t, err)
