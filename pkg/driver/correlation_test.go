...
func TestAdmissionEventMessageNamesTheNUMANode(t *testing.T) {
	node := 0
	corr := store.ClaimCorrelation{NUMANode: &node, Partition: "dpdk", InitialCPUSet: "6-7", FrontierSnapshot: "-,-,-,-"}
	require.Equal(t, "admitted on NUMA 0 partition dpdk with cpuset 6-7 frontier -,-,-,- witness ", admissionEventMessage(corr))

	corr.NUMANode = nil
	require.Contains(t, admissionEventMessage(corr), "admitted on NUMA unknown partition")
}
