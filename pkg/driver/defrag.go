...
func (cp *CPUDriver) settleExchanges(logger logr.Logger, round *defragRound, failed []*api.ContainerUpdate) {
	...
	for _, step := range defragSteps(round.moves) {
		...
		round.outcomes[exchange] = cp.rollBackExchange(eLogger, round, exchange, refused)
	}
}
...
// exchange undone or unsettled.
func (cp *CPUDriver) rollBackExchange(logger logr.Logger, round *defragRound, exchange int, refused map[types.UID]struct{}) exchangeOutcome {
	updates, err := cp.rollbackUpdates(round, exchange, refused)
	if err != nil {
...
func (cp *CPUDriver) rollbackUpdates(round *defragRound, exchange int, refused map[types.UID]struct{}) ([]*api.ContainerUpdate, error) {
	cp.applyMu.Lock()
	...
	for _, containerUID := range round.exchangeContainers[exchange] {
		if _, wasRefused := refused[containerUID]; wasRefused {
			continue
		}
...
func (cp *CPUDriver) observeNodeShape(logger logr.Logger, online cpuset.CPUSet) {
	...
	for _, numaNodeID := range topo.CPUDetails.NUMANodes().List() {
		cp.metricsRecorder().SetDefragNodePoisoned(numaNodeID, cp.nodeIsPoisoned(numaNodeID))
