...
func (cp *CPUDriver) sendDefragBatch(logger logr.Logger, updates []*api.ContainerUpdate) ([]*api.ContainerUpdate, error) {
	...
	go func() {
		...
		// Cleared before the reply is delivered, not after it: a caller that sends
		// the next batch as soon as this one answers must not find it still set.
		cp.defragBatchInFlight.Store(false)
