...
// errDefragBatchInFlight is returned when the previous batch has neither
// answered nor failed. It reads to a caller exactly as a timeout does: the round
// is kept and attempted again.
var errDefragBatchInFlight = errors.New("a batch of container updates is still out")
...
//
// One batch is out at a time. Giving up on a call does not end it, so a runtime
// that stops answering would otherwise be sent a new batch on every retry and
// leave a goroutine blocked in each, for the life of the process. Refusing to
// start a second is also the truthful answer: the first may yet be applied, and
// sending more updates while that is unknown is how two cpusets end up written
// for one container.
//
...
func (cp *CPUDriver) sendDefragBatch(logger logr.Logger, updates []*api.ContainerUpdate) ([]*api.ContainerUpdate, error) {
	...
	if !cp.defragBatchInFlight.CompareAndSwap(false, true) {
		logger.Error(nil, "a batch of container updates is still out, not sending another", "numUpdates", len(updates))
		return nil, errDefragBatchInFlight
	}
	type reply struct {
	...
	go func() {
		defer cp.defragBatchInFlight.Store(false)
...
func (cp *CPUDriver) reconcileMakeRoomTargets(ctx context.Context) {
	// A make-room target is a promise to move claims out of a cache so that a
	// waiting one fits. Nothing keeps that promise with defragmentation off, and
	// the claim reader is now built whether or not the feature is on, so the
	// guard has to be the feature rather than the reader being absent.
	if !cp.defrag.enabled || cp.cpuDeviceGroupBy != device.GROUP_BY_UNCORE_CACHE || cp.claimReader == nil {
		return
	}
...
func (cp *CPUDriver) finishDefragRound(logger logr.Logger, round *defragRound, failed []*api.ContainerUpdate, updateErr error) cpumetrics.Result {
	...
	for _, step := range defragSteps(round.moves) {
		...
		if step[0].Exchange == 0 {
			...
			cp.publishClaimPlacementStatus(context.Background(), sLogger, move.ClaimUID)
			committed++
			continue
		}
...
func (cp *CPUDriver) settleExchangeStep(logger logr.Logger, round *defragRound, step []defrag.Move) bool {
	...
	case exchangeApplied:
		...
		for _, move := range step {
			...
			cp.publishClaimPlacementStatus(context.Background(), logger, move.ClaimUID)
		}
		return true
	case exchangeUndone:
