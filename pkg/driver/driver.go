...
type CPUDriver struct {
	...
	// defragBatchInFlight is set while a batch of container updates is out. The
	// NRI stub calls the runtime with a background context and no timeout
	// (UpdateContainers in github.com/containerd/nri/pkg/stub), so a call this
	// driver has stopped waiting for cannot be cancelled and its goroutine stays
	// blocked until the runtime answers, if it ever does.
	defragBatchInFlight atomic.Bool
...
func New(logger logr.Logger, providers Providers, config *Config) (*CPUDriver, error) {
	...
	plugin.cpuAllocationStore.SetClaimlessCPUs(plugin.defaultPartitionCPUs)
	...
	// The fence is a map every reader of it consults and every writer assumes,
	// and a nil map reads as empty but panics on write. Built here rather than
	// beside the defragmentation machinery below, because "nothing can fence a
	// node while the feature is off" is an argument about today's callers, and
	// the cost of not relying on it is one allocation.
...
func verifyThreadArity(topo *cpuinfo.CPUTopology, partition device.Partition) string {
	...
	files := make([]string, 0, surplus.Size())
	for _, cpuID := range surplus.List() {
		files = append(files, fmt.Sprintf("/sys/devices/system/cpu/cpu%d/online", cpuID))
	}
	return fmt.Sprintf("partition %q expects at most %d online thread(s) per core, but %s are online too; offline them by writing 0 to %s",
		partition.Name, partition.ThreadsPerCore, surplus.String(), strings.Join(files, ", "))
}
