...
func New(logger logr.Logger, providers Providers, config *Config) (*CPUDriver, error) {
	...
	if plugin.cpuDeviceMode == device.CPU_DEVICE_MODE_GROUPED {
		built := device.BuildGrouped(logger, plugin.cpuDeviceGroupBy, plugin.topology.cpuTopology, plugin.topology.reservedCPUs, plugin.pcieRootMapper, config.PublishNodeAllocatableResourceMapping, config.FullPhysicalCPUsOnly, plugin.partitions)
