...
func (r *oracleClaimReader) GetProjectedClaims() (*v1alpha1.ProjectedClaims, error) {
	...
	defer r.mu.RUnlock()
	if r.projected == nil {
		// An empty projection, not a nil one: to every caller "nothing has been
		// projected" and "the projection names nothing" are the same answer, and
		// the fake exists to drive the callers.
...
func newOracleHarness(t *testing.T, cfg oracleTopologyConfig) *oracleHarness {
	...
	for i := range totalCaches {
		switch {
		case len(cfg.cpusPerCache) == 1:
			cacheSizes[i] = cfg.cpusPerCache[0]
		case i < len(cfg.cpusPerCache):
	...
	d := &CPUDriver{
		...
		claimReader:      reader,
		cpuAllocator:     cpuallocator.NewCPUManager("dra.cpu", topo),
...
func (h *oracleHarness) CrashAndRestart() {
	...
	allCPUs := h.baseTopology.cpuTopology.CPUDetails.CPUs()
	...
	newD := &CPUDriver{
		...
		cpuAllocator:     cpuallocator.NewCPUManager(oldD.driverName, topo),
...
func TestInvariantOracle_UnequalCacheSizesAgainstUncanonicalisedOracle(t *testing.T) {
	...
	dTopo, err := defrag.NewTopology(topo, 0, h.driver.topology.cpuTopology.CPUDetails.CPUs())
	require.NoError(t, err)
