...
// TestNoMakeRoomTargetWithDefragmentationOff: a make-room target is a promise to
// move claims out of a cache so a waiting one fits, and with defragmentation off
// nothing on the node keeps it. The claim reader is built whether or not the
// feature is on, so the reader being present is no longer evidence that it is.
func TestNoMakeRoomTargetWithDefragmentationOff(t *testing.T) {
	d := newDefragTestDriver(t, 2, 4)
	...
	d.topology.deviceNameToUncoreCacheID = map[string]int{"cache-0": 0, "cache-1": 1}
	d.topology.deviceNameToNUMANodeID = map[string]int{"cache-0": 0, "cache-1": 0}
			...
			Claims: []v1alpha1.ProjectedClaim{
				{
					...
					State:     v1alpha1.ClaimStateAllocated,
					Shape:     "never-split",
					Devices:   []v1alpha1.ProjectedDevice{{Device: "cache-0"}},
				},
			},
		},
	}
	...
	d.applyMu.Lock()
	defer d.applyMu.Unlock()
	require.Empty(t, d.makeRoomTargets)
}
