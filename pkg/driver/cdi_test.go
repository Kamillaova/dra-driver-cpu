...
func TestGetDeviceAllocationsRoundTripsAPoolRequest(t *testing.T) {
	...
	require.NoError(t, mgr.AddDevice(logger, deviceName, "DRA_CPUSET_claim-cpu-pool=0-1,6-7", record))
	require.NoError(t, mgr.Refresh())
...
// TestGetDeviceAllocationsRefusesUnusablePlacements: an annotation that parses
// as JSON but describes no usable placement. Each of these would otherwise be
// accepted silently and read later as a claim holding no CPUs of its own.
func TestGetDeviceAllocationsRefusesUnusablePlacements(t *testing.T) {
	for _, tc := range []struct {
		...
		name:       "no role at all",
		annotation: `[{"request":"cpus","cpus":"0-1"}]`,
		wantError:  "unrecognised role",
	}} {
		t.Run(tc.name, func(t *testing.T) {
			logger := testr.New(t)
			...
			deviceName := "claim-cpu-unusable"
			spec := &cdiSpec.Spec{
				Version: cdiSpecVersion,
				Kind:    cdiVendor + "/" + cdiClass,
				Devices: []cdiSpec.Device{{
					Name:        deviceName,
					Annotations: map[string]string{cdiPlacementsAnnotation: tc.annotation},
					ContainerEdits: cdiSpec.ContainerEdits{
						Env: []string{"DRA_CPUSET_claim-cpu-unusable=0-1"},
					},
				}},
			}
			require.NoError(t, mgr.cache.WriteSpec(spec, mgr.getSpecName(deviceName)))
			...
			_, err = mgr.GetDeviceAllocations(deviceName)
			require.Error(t, err, "the env var must not be used as a fallback: the annotation is present and wrong")
