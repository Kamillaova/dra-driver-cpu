...
func deviceDiffers(want, got resourceapi.Device) string {
	...
	case !taintsAgree(want.Taints, got.Taints):
...
// taintsAgree compares the taints the driver means against the stored ones,
// which carry a timestamp the driver never published: the API server adds
// TimeAdded to a taint that arrives without one, so comparing it would leave
// every tainted device disagreeing with itself for as long as the driver runs
// -- and every device of a named partition is tainted.
//
// Everything a taint does is still compared. A changed key, value or effect is
// a device the driver no longer means, and a round must not move a claim onto
// it.
func taintsAgree(want, got []resourceapi.DeviceTaint) bool {
	if len(want) != len(got) {
		return false
	}
	for i := range want {
		w, g := want[i], got[i]
		w.TimeAdded, g.TimeAdded = nil, nil
		if !apiequality.Semantic.DeepEqual(w, g) {
			return false
		}
	}
	return true
