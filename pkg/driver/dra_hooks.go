...
func (cp *CPUDriver) PrepareResourceClaims(ctx context.Context, claims []*resourceapi.ResourceClaim) (map[types.UID]kubeletplugin.PrepareResult, error) {
	...
	for _, claim := range claims {
		...
		// still starting; a prepare is also the moment the shape the fit
		// annotation describes changes. Not the shared reconcile: after a prepare
		// there is nothing for it to do that CreateContainer will not.
		if (cp.defrag.enabled || cp.fit.enabled) && result[claim.UID].Err == nil {
			cp.requestReconcile()
		}
	}
	return result, nil
