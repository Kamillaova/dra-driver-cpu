...
func (cp *CPUDriver) prepareGroupedResourceClaim(ctx context.Context, logger logr.Logger, claim *resourceapi.ResourceClaim, placement opaqueapi.ClaimPlacement) kubeletplugin.PrepareResult {
	...
	cp.recordClaimEvent(ctx, claim, "ClaimAdmitted", admissionEventMessage(corr))
...
// print an address.
func admissionEventMessage(corr store.ClaimCorrelation) string {
	numaNode := "unknown"
	if corr.NUMANode != nil {
		numaNode = fmt.Sprintf("%d", *corr.NUMANode)
	}
	return fmt.Sprintf("admitted on NUMA %s partition %s with cpuset %s frontier %s witness %s",
		numaNode, corr.Partition, corr.InitialCPUSet, corr.FrontierSnapshot, corr.WitnessPlan)
}
