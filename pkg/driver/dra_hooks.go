...
func (cp *CPUDriver) prepareGroupedResourceClaim(ctx context.Context, logger logr.Logger, claim *resourceapi.ResourceClaim, placement opaqueapi.ClaimPlacement) kubeletplugin.PrepareResult {
	...
	uncoreCaches := cp.uncoreCachesOf(assignedCPUs)
	logger.Info("claim admitted",
		"claimUID", claim.UID,
		"numaNode", corr.NUMANode,
		"uncoreCaches", uncoreCaches,
...
func (cp *CPUDriver) recordedDeviceFull(ctx context.Context, logger logr.Logger, claim *resourceapi.ResourceClaim, deviceName string, room, charged int, cause error) error {
	...
	if !claimOffersSplitAlternatives(claim) {
		...
		if cp.defrag.enabled && cp.cpuDeviceGroupBy == device.GROUP_BY_UNCORE_CACHE {
...
func (cp *CPUDriver) prepareResourceClaim(ctx context.Context, logger logr.Logger, claim *resourceapi.ResourceClaim, placement opaqueapi.ClaimPlacement) kubeletplugin.PrepareResult {
	...
	uncoreCaches := cp.uncoreCachesOf(claimCPUSet)
	logger.Info("claim admitted",
		"claimUID", claim.UID,
		"numaNode", corr.NUMANode,
		"uncoreCaches", uncoreCaches,
...
// A claim that needs no witness yields no plan and no error: that is the answer,
// not a missing one, and every caller reads a nil plan as "nothing to hold".
//
//nolint:nilnil // the pair is the result, not an omission
func (cp *CPUDriver) secureRepairWitness(ctx context.Context, logger logr.Logger, claim *resourceapi.ResourceClaim, cpuAssignment cpuset.CPUSet, placement opaqueapi.ClaimPlacement) (*defrag.ExactPlan, error) {
	...
	// A repairable claim that landed split is being admitted against a repair,
	// and where nothing defragments there is no repair to admit it against. The
	// witness below would prove a plan no pass will ever run, reserve the CPUs
	// that plan needs against claims that could have used them, and leave the
	// workload split for the life of its pod with nothing recorded to say why.
	if !cp.defrag.enabled {
		cp.recordClaimEvent(ctx, claim, "NoRepairer",
			"claim asks to be repaired if it lands split, and this node does not defragment")
		cp.metrics.RecordPrepareNoWitness()
		return nil, fmt.Errorf("repairable claim %s/%s landed split on NUMA node %d and defragmentation is disabled on this node",
			claim.Namespace, claim.Name, numaNodeID)
	}
...
func (cp *CPUDriver) buildClaimCorrelation(claim *resourceapi.ResourceClaim, cpus cpuset.CPUSet, plan *defrag.ExactPlan) store.ClaimCorrelation {
	...
	if plan != nil {
		// planRounds rather than a move count, because this number sits in the
		// claim status beside the frontier the claim was admitted against, and
		// the frontier counts rounds: an exchange is two moves in one of them,
		// so counting moves makes the pair disagree exactly when a plan swaps.
		r := planRounds(plan.Moves)
		witnessRounds = &r
		var descs []string
...
// admissionEventMessage is the ClaimAdmitted event's text. The NUMA node is a
// pointer, absent unless the claim sits in exactly one node, and %v on it would
// print an address.
func admissionEventMessage(corr store.ClaimCorrelation, uncoreCaches []int) string {
	numaNode := "unknown"
	if corr.NUMANode != nil {
		numaNode = fmt.Sprintf("%d", *corr.NUMANode)
	}
	caches := "unknown"
	if len(uncoreCaches) > 0 {
		ids := make([]string, len(uncoreCaches))
		for i, id := range uncoreCaches {
			ids[i] = fmt.Sprintf("%d", id)
		}
		caches = strings.Join(ids, ",")
	}
	return fmt.Sprintf("admitted on NUMA %s caches %s partition %s with cpuset %s frontier %s witness %s",
		numaNode, caches, corr.Partition, corr.InitialCPUSet, corr.FrontierSnapshot, corr.WitnessPlan)
}
