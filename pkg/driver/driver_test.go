...
func TestNewDegradesAPartitionTheMachineContradicts(t *testing.T) {
	...
	require.Contains(t, cp.degradedPartitions["dataplane"], "/sys/devices/system/cpu/cpu5/online",
		"the message names the file that would make the declaration true")
...
func TestVerifyThreadArityAcceptsWhatItWasNotToldAbout(t *testing.T) {
	...
	require.Equal(t,
		`partition "vm" expects at most 1 online thread(s) per core, but 5 are online too; offline them by writing 0 to /sys/devices/system/cpu/cpu5/online`,
		verifyThreadArity(topo, devattr.Partition{Name: "vm", CPUs: cpuset.New(1), ThreadsPerCore: 1}))
}
