...
func TestPlacementsPlansWithApplyMuReleased(t *testing.T) {
	// A remote poller of this endpoint must not stall Prepare or CreateContainer
	// behind its planning, so the snapshot is what the lock covers and the plan
	// is computed after it. Asserted structurally rather than by timing: the
	// snapshot hands back its planning inputs, and the lock is free once it has.
	d := newDefragTestDriver(t, 2, 4)
	d.placeClaim(t, "claim-1", cpuset.New(0, 4))
	d.runContainer(t, "pod-uid-1", "ctr-1", "ctr-uid-1", "claim-1")
