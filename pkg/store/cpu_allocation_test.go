...
func TestReserveRefusesAClaimWithNoRequests(t *testing.T) {
	...
	require.Error(t, store.ReserveResourceClaimAllocation(logger, "claim-1", nil, false))
	require.Error(t, store.ReserveResourceClaimAllocation(logger, "claim-1", []RequestAllocation{}, false))

	_, ok := store.GetResourceClaimRequests("claim-1")
