...
func TestPlanNodeHonoursTheMoveCap(t *testing.T) {
	topo := topologyOf([]int{4, 4, 4, 4})
	dtopo := requireTopology(t, topo, 0, topo.CPUDetails.CPUs())
	...
	uncapped, err := defrag.PlanNode(dtopo, placements, free, selectorFor(topo), defrag.Options{})
	require.NoError(t, err)
	require.Greater(t, len(uncapped.Moves), 1)

	capped, err := defrag.PlanNode(dtopo, placements, free, selectorFor(topo), defrag.Options{MaxMoves: 1})
	require.NoError(t, err)
	require.Len(t, capped.Moves, 1)
	require.Equal(t, uncapped.Moves[0], capped.Moves[0], "the cap keeps the most useful move")
	require.Contains(t, capped.Reason, "move cap")
	requireValidPlan(t, dtopo, placements, free, capped)

	// Capping only slows a pass down; it must still converge.
	_, final, _ := converge(t, topo, dtopo, placements, free, defrag.Options{MaxMoves: 1})
	require.Equal(t, 0, dtopo.Cost(final))
}

func TestPlanNodeHonoursMinGain(t *testing.T) {
	topo := topologyOf([]int{4, 4, 4, 4})
	dtopo := requireTopology(t, topo, 0, topo.CPUDetails.CPUs())
	...
	worthOne, err := defrag.PlanNode(dtopo, placements, free, selectorFor(topo), defrag.Options{})
	require.NoError(t, err)
	require.Equal(t, 1, worthOne.CurrentCost-worthOne.IdealCost)
	require.NotEmpty(t, worthOne.Moves)

	tooSmall, err := defrag.PlanNode(dtopo, placements, free, selectorFor(topo), defrag.Options{MinGain: 2})
	require.NoError(t, err)
	require.Empty(t, tooSmall.Moves)
	require.Contains(t, tooSmall.Reason, "below the minimum of 2")
}

func TestPlanNodeTreatsIneligibleClaimsAsObstacles(t *testing.T) {
	// A claim under cooldown must not be moved, and the ideal has to be built
...
func TestPlanNodeMovesOnlyTheClaimsARepairNeeds(t *testing.T) {
	...
	for pass := 0; pass < 10; pass++ {
		plan, err := defrag.PlanNode(dtopo, current, free, selectorFor(topo), defrag.Options{MaxMoves: 4})
		require.NoError(t, err)
