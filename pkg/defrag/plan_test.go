...
func TestPlanNodeKeepsTheSharedPoolNonEmpty(t *testing.T) {
	...
	// The guard refuses a move into free CPUs. An exchange takes none, so it is
	// still open, and a node that keeps its pool is not thereby left fragmented.
	swappable, err := defrag.PlanNode(dtopo, placements, free, selectorFor(topo),
		defrag.Options{KeepFreePoolNonEmpty: true, AllowSwaps: true})
	require.NoError(t, err)
	requireValidPlan(t, dtopo, placements, free, swappable)
	require.NotEmpty(t, swappable.Moves, "an exchange needs no free CPU and so survives the pool guard")
	require.False(t, free.Difference(targetsIn(swappable)).IsEmpty(), "no CPU is left for shared containers")

	require.Contains(t, guarded.Reason, "transient overlap",
		"the free-CPU move is held back by the pool, and what stops the exchange behind it is the swap setting")
}
