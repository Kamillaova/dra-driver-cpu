...
type Options struct {
	// MaxMoves caps the number of moves. Zero means no cap. A pass covering
	// several NUMA nodes passes what is left of its budget for each node.
	MaxMoves int
	// MinGain is the smallest total cost improvement worth moving for. Anything
	// below 1 is treated as 1, so a node already at its ideal is never
	// disturbed.
	MinGain int
	// Eligible reports whether a claim may be moved at all, which is how a
	// cooldown or an opt-out is applied. Nil means every claim may move.
	//
...
func PlanNode(topo *Topology, placements []Placement, free cpuset.CPUSet, sel Selector, opts Options) (Plan, error) {
	...
	minGain := max(opts.MinGain, 1)
	if gain := plan.CurrentCost - plan.IdealCost; gain < minGain {
		if gain <= 0 {
			plan.Reason = "node is already packed as well as its claims allow"
		} else {
			plan.Reason = fmt.Sprintf("cost gain of %d is below the minimum of %d", gain, minGain)
		}
		return plan, nil
	}
...
func plannedMoves(topo *Topology, placements []Placement, ideal map[types.UID]cpuset.CPUSet, free cpuset.CPUSet, sel Selector, opts Options) ([]Move, int, string) {
	...
	for _, p := range packingOrder(placements) {
		...
		if opts.MaxMoves > 0 && len(moves) >= opts.MaxMoves {
			blocked++
			reason = fmt.Sprintf("move cap of %d reached", opts.MaxMoves)
			continue
		}
