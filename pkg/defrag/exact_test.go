...
// TestDepthCutoffIsNotProofOfUnreachability: the three-move counterexample with
// a budget of two. An empty queue proves nothing about the states the cutoff
// threw away, and calling that unreachable would have the frontier publish a
// dash for a repair that exists.
func TestDepthCutoffIsNotProofOfUnreachability(t *testing.T) {
	topo, cpuTopo := fakeTopologyWithCPUInfo(4, 16)
	placements := []Placement{
		{ClaimUID: types.UID("claim-X"), CPUs: parseCPUSet("0-3,16-27")},
		{ClaimUID: types.UID("claim-B"), CPUs: parseCPUSet("4-15")},
		{ClaimUID: types.UID("claim-S-immobile"), CPUs: parseCPUSet("28-31")},
		{ClaimUID: types.UID("claim-Y"), CPUs: parseCPUSet("32-39")},
		{ClaimUID: types.UID("claim-D-immobile"), CPUs: parseCPUSet("44-47")},
		{ClaimUID: types.UID("claim-E-immobile"), CPUs: parseCPUSet("48-55")},
	}
	free := parseCPUSet("40-43,56-63")
	eligible := func(uid types.UID) bool {
		return uid != "claim-S-immobile" && uid != "claim-D-immobile" && uid != "claim-E-immobile"
	...
	deep, err := ExactSearch(topo, placements, free, cpuset.New(), goal, sel, ExactOptions{Eligible: eligible, MaxDepth: 3})
	if err != nil {
		t.Fatalf("search error: %v", err)
	}
	if deep.Status != SearchReachable {
		t.Fatalf("the repair takes three moves and three were allowed: got %v", deep.Status)
	}

	shallow, err := ExactSearch(topo, placements, free, cpuset.New(), goal, sel, ExactOptions{Eligible: eligible, MaxDepth: 2})
	if err != nil {
		t.Fatalf("search error: %v", err)
	}
	if shallow.Status != SearchNotProven {
		t.Fatalf("a repair the cutoff hid must read not-proven, not %v", shallow.Status)
	}
}
...
// and two separate claims cannot be made into one at all -- and sharing a key
// makes the search prune the second as already seen.
func TestStateKeyTellsOneSplitClaimFromTwoWholeOnes(t *testing.T) {
	topo := fakeTopology(2, 16)
	free := parseCPUSet("2-15,18-31")
	opts := ExactOptions{Eligible: func(types.UID) bool { return true }}
	goal := GoalMakeClaimWhole{ClaimUID: "claim-1"}

	split := map[types.UID]cpuset.CPUSet{
		"claim-1": parseCPUSet("0-1,16-17"),
	}
	apart := map[types.UID]cpuset.CPUSet{
		"claim-1": parseCPUSet("0-1"),
		"claim-2": parseCPUSet("16-17"),
	}

	for _, canonical := range []bool{true, false} {
		splitKey := stateKey(topo, split, free, goal, opts, canonical)
		apartKey := stateKey(topo, apart, free, goal, opts, canonical)
		if splitKey == apartKey {
			t.Fatalf("canonical=%t: a split claim and two whole ones share the key %q", canonical, splitKey)
		}
	}

	// The point of canonicalisation still holds: relabelling the caches is not a
	// different state.
	mirrored := map[types.UID]cpuset.CPUSet{
		"claim-1": parseCPUSet("16-17,0-1"),
	}
	if a, b := stateKey(topo, split, free, goal, opts, true), stateKey(topo, mirrored, free, goal, opts, true); a != b {
		t.Fatalf("the same state read two ways gave two keys:\n%q\n%q", a, b)
	}
}
