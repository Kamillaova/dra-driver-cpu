	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.cpuDeviceMode == device.CPU_DEVICE_MODE_GROUPED {
				devices, _, _ = device.BuildGrouped(logr.Discard(), tc.groupBy, topo, reserved, store.NewPCIeRootMapper(), tc.publishNodeAllocatableMapping, false)
			} else {
...
func TestMachineGroupedUsesTopologyValidatedCPUs(t *testing.T) {
	...
	devices, _, _ := device.BuildGrouped(
		logr.Discard(), device.GROUP_BY_MACHINE, topo, cpuset.New(),
		store.NewPCIeRootMapper(), false, false,
	)
...
func TestGroupedFullPhysicalCPUsOnlyIsANoOpWithoutSMT(t *testing.T) {
	...
	devices, _, deviceThreadsPerCore := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
		cpuset.New(), store.NewPCIeRootMapper(), false, true)
	require.Len(t, devices, 1)
