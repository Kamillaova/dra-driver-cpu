	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.cpuDeviceMode == device.CPU_DEVICE_MODE_GROUPED {
				built := device.BuildGrouped(logr.Discard(), tc.groupBy, topo, reserved, store.NewPCIeRootMapper(), tc.publishNodeAllocatableMapping, false, unpartitioned(topo.CPUDetails.CPUs()))
...
func TestMachineGroupedUsesTopologyValidatedCPUs(t *testing.T) {
	...
	built := device.BuildGrouped(
		logr.Discard(), device.GROUP_BY_MACHINE, topo, cpuset.New(),
		store.NewPCIeRootMapper(), false, false, unpartitioned(topo.CPUDetails.CPUs()),
	)
	devices := onlyPartition(t, built.ByPartition)
...
func TestGroupedUncoreCacheAttributes(t *testing.T) {
	...
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
		cpuset.New(0), store.NewPCIeRootMapper(), false, false, unpartitioned(topo.CPUDetails.CPUs().Difference(cpuset.New(0))))
	devices := onlyPartition(t, built.ByPartition)
	require.Len(t, devices, 1)
...
func TestGroupedUncoreCacheAttributesOmittedWhenUnknown(t *testing.T) {
	...
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
		cpuset.New(), store.NewPCIeRootMapper(), false, false,
...
func TestGroupedUncoreCacheDevices(t *testing.T) {
	...
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_UNCORE_CACHE, topo,
...
func TestGroupedFullPhysicalCPUsOnly(t *testing.T) {
	...
	buildOne := func(t *testing.T, reserved cpuset.CPUSet, fullPhysicalCPUsOnly bool) resourceapi.Device {
		t.Helper()
		built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
			reserved, store.NewPCIeRootMapper(), false, fullPhysicalCPUsOnly, unpartitioned(topo.CPUDetails.CPUs().Difference(reserved)))
		devices := onlyPartition(t, built.ByPartition)
		require.Len(t, devices, 1)
		return devices[0]
	}
...
func TestGroupedFullPhysicalCPUsOnlyIsANoOpWithoutSMT(t *testing.T) {
	...
	topo := fakeCacheTopology()
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
		cpuset.New(), store.NewPCIeRootMapper(), false, true, unpartitioned(topo.CPUDetails.CPUs()))
	devices := onlyPartition(t, built.ByPartition)
	require.Len(t, devices, 1)
...
func TestGroupedThreadsPerCoreIndependentOfFullPhysicalCPUsOnly(t *testing.T) {
	...
	topo := fakeSMTCacheTopology()
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, topo,
		cpuset.New(), store.NewPCIeRootMapper(), false, false, unpartitioned(topo.CPUDetails.CPUs()))
	devices := onlyPartition(t, built.ByPartition)
	require.Len(t, devices, 1)
...
func TestGroupedNonUniformDeviceHasNoSMTAttribute(t *testing.T) {
	...
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_NUMA_NODE, hybridTopo,
		cpuset.New(), store.NewPCIeRootMapper(), false, true, unpartitioned(hybridTopo.CPUDetails.CPUs()))
	devices := onlyPartition(t, built.ByPartition)
	require.Len(t, devices, 1)
...
func TestPoolPartitionPublishesOneDevicePerNUMANode(t *testing.T) {
	...
	built := device.BuildGrouped(logr.Discard(), device.GROUP_BY_MACHINE, topo,
