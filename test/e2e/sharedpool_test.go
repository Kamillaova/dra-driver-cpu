...
var _ = ginkgo.Describe("Static shared pool", ginkgo.Serial, ginkgo.Ordered, ginkgo.ContinueOnFailure, func() {
	...
	ginkgo.It("should pin claimless containers to the pool and never re-pin them", func(ctx context.Context) {
		...
		gomega.Eventually(observeAssignedCPUs(ctx, fxt, sharedPod)).WithTimeout(1*time.Minute).WithPolling(5*time.Second).Should(cpusetmatchers.Equal(staticPool), "the claimless pod %s is not confined to the static pool", e2epod.Identify(sharedPod))
