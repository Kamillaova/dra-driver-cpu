	...
	ginkgo.It("should never over-allocate under a burst of racing claims", func(ctx context.Context) {
		...
		for _, dev := range fleet {
			if dev.node == target {
				...
				spare += dev.capacity % size
			}
		}
		// A claim may not take the node's last free CPU while a container
		// holding none is running, and with CNI on the node one always is, so
		// devices that divide exactly cost the burst its final claim.
		if spare == 0 {
			fit--
		}
		burst := fit + 3
